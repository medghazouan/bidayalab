...
export default function BlogListingPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      setLoading(true);
      const { data } = await getBlogs('all'); // Fetch all blogs
      setPosts(data || []);
      setLoading(false);
    }
    fetchData();
        ...
        <div className="relative z-10 max-w-7xl w-full mx-auto">
          ...
          >
            <h1 className="text-[12vw] md:text-[10vw] font-bold font-louis leading-[0.85] tracking-tighter uppercase text-white mix-blend-difference">
              The <span className="text-zinc-700">Journal</span>
            </h1>
            <p className="mt-8 text-lg md:text-xl text-zinc-400 max-w-xl font-sans border-l-2 border-[#beff01] pl-6">
              Deep dives into design, technology, and the architecture of digital dominance.
            </p>
          </motion.div>
        </div>
      </section>
